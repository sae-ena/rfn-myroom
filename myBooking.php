...
.parentBtn {
    display: flex;
    justify-content: space-between; /* This makes sure space between left and right is maximized */
...
.warning-message {
    ...
    transform: translate(-50%, -50%); /* Offset the element by 50% of its width and height */
    padding: 20px;
    background-color:rgb(241, 238, 11); /* Light red background for warning */
    color: #721c24; /* Dark red color for text */
    border: 1px solid #f5c6cb; /* Border to match the warning style */
    border-radius: 5px; /* Rounded corners */
    font-size: 18px;
    font-weight: bold;
    text-align: center;
    box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); /* Subtle shadow */
    z-index: 1000; /* Make sure the message is on top of other content */
}
...
if(! $result) $form_error = "Failed to Cancel the booking";
...
$result = $conn->execute_query($query);
    ...
    <form action="myBooking.php" method="GET" class="status-filter-form">
        ...
        <option value="canceled" <?php if (isset($_GET['status']) && $_GET['status'] == 'canceled') echo 'selected'; ?>>Canceled</option>
    </select>
...
<?php
    ...
    <?= (strlen($row['room_description']) > 220) ? substr($row['room_description'], 0, 220) . '...' : $row['room_description'] ?>
                    ...
                    <div class="parentBtn">
                        ...
                        <a href="booking_details.php?booking_id=<?= $row['booking_id'] ?>" class="view-details">View Details</a>
