...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register'])) {
    $user_name = $user_email = $user_number = $user_password = $user_confirm_password= $user_location = "";
    // Get form data
    ...
    // Validate input
    if (empty($user_name) || empty($user_email) || empty($user_password) || empty($user_location)) {
...
elseif (strlen($user_password) < 5 || strlen($user_confirm_password) < 5) {
...
elseif(! isset($form_error)){
    $uniqueCheck = "SELECT user_number FROM users WHERE user_number = '$user_number'";
    $result = $conn->query($uniqueCheck);
        if($result->num_rows > 0){
    $form_error = "Phone Number Already Exists";
}
}
elseif(! isset($form_error) ){
    $prefixes = ['98', '99', '97']; // Array of prefixes to check
    $is_invalid = in_array(true, array_map(function($prefix) use ($user_number) {
        return str_starts_with($user_number, $prefix);
    }, $prefixes));
  
if(! $is_invalid) $form_error ="Number Must Start with 97|98|95";
}
if(! isset($form_error)){
        // Hash the password
        ...
        $query = "INSERT INTO users (user_name, user_email, user_number, user_location, user_password)
                  VALUES ('$user_name', '$user_email', '$user_number','$user_location', '$hashed_password')";
           $sqlResult = InsertRoomData::insertData($query);
           $successfullyRoomAdded = "User registered successfully!";

           $user_name = $user_number = $user_email = $user_location ="";
  ...
  <div style="width: 40%; background-color:#60BB46; display: flex; justify-content: center; align-items: center; margin: 1px 39px; padding: 0; border-radius: 54px 21px;">
    <div style="text-align: center; width: 80%; max-width: 400px; margin: 0; padding: 20px; border-radius: 10px;">
      ...
      <h2>Sign Up</h2>
        ...
        <input type="text" name="user_location" value="<?php echo isset($user_location)? htmlspecialchars($user_location):""; ?>" placeholder="Address " style="padding: 10px; border: 1px solid #ccc; border-radius: 5px;">
